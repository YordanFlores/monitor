...
/** Límite de tamaño del .bin (ajustar si el build es mayor). */
...
function publicFirmwareUrl(request: Request, pathname: string): string {
  const host =
...
/**
 * POST /api/firmware/upload?unit=UNIDAD_01
 * multipart/form-data: campo `file` = firmware .bin
 * Respuesta: { ok: true, path: "/firmware/UNIDAD_01.bin", url: "https://..." }
 */
export async function POST(request: Request) {
  ...
  const dir = path.join(root, "public", "firmware");
  const fsPath = path.join(dir, filename);

  await mkdir(dir, { recursive: true });
  await writeFile(fsPath, buf);

  const webPath = `/firmware/${filename}`;
  const url = publicFirmwareUrl(request, webPath);
